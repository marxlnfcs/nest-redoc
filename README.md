<p align="center" style="font-size: 40px;">NestJS ReDoc</p>

<p align="center">A NestJS Frontend for your OpenAPI Specs powered by <a href="https://redocly.com/" target="_blank">ReDoc</a></p>
<p align="center">
...
run = () => {
    ...
    const config = RedocModule.createDocumentBuilder() // OR: new DocumentBuilder()
      .setTitle('Cats');
    
    // create the document from the config
    const document = RedocModule.createDocument(app, config.build()); // OR: SwaggerModule.createDocument(app, config.build())
